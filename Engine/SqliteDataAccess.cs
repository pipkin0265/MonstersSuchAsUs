...
namespace Engine
{
    ...
    { public static void BuildWorld()
        ...
        public List<Item> SaveItems(Item item)
        {
            ...
            {
                conn.Execute("", item);
                var output = conn.Query<Item>("select * Items", new DynamicParameters());
                return output.ToList();
            }
        }
        ...
        public List<Item> SaveTreasure(Item Treasure)
        {
            using (IDbConnection conn = new SQLiteConnection(LoadConncectionString()))
            {
                ...
                var output = conn.Query<Item>("select * Treasure", new DynamicParameters());
                return output.ToList();
            }
        }
        ...
        public static List<Item> GetWeapon()
        {
            using (IDbConnection conn = new SQLiteConnection(LoadConncectionString()))
            {
                ...
                var output = conn.Query<Item>("select * from Weapons", new DynamicParameters());
                return output.ToList();
            }
        }
